...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      await addCourseAndPrerequisitesToStudyplan(
        plan_id,
        courseCode,
        prerequisiteCodes
      );
      res
        .status(200)
        .json({ message: "Courses added to study plan successfully" });
    } catch (error) {
...
router.get(
  "/databaseGetCourseWithRequirements/:plan_id/:course_id",
  asyncHandler(async (req, res) => {
    const { plan_id, course_id } = req.params;
    const courseRequirements = await getCourseWithReqursivePrerequisites(
      plan_id,
      course_id
    );
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const newPrerequisite = await addPrerequisiteCourse(
      course_kori_name,
      prerequisite_course_kori_name
    );
...
router.get(
  "/getCourseWithPrerequisites/:course_kori_name",
  asyncHandler(async (req, res) => {
    ...
    if (courseGraph) {
      ...
      res
        .status(404)
        .send({ message: "Course not found or it has no prerequisites." });
    }
  })
);
