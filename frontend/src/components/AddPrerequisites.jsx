  ...
  const addPrerequisites = async (event) => {
    ...
    const prerequisitesObject = {
      ...
      prerequisiteCodes: prerequisiteCodes
        ? prerequisiteCodes.split(",").map((code) => code.trim())
        : [],
    };

    try {
      const response = await axiosInstance.post(
        "/api/courses/addCourseToStudyplan",
        prerequisitesObject
      );
  ...
  return (
    <div className="prerequisites-view">
      ...
      <form onSubmit={addPrerequisites}>
        <div>
          <label className="form-label-give-course">
            Anna lisättävän kurssin kurssikoodi:
          </label>
          <input
          ...
          />
          <label className="form-label-course-prerequisites">
            Anna kurssille esitietokurssit kurssikoodeilla:
          </label>
          <input
            className="form-input"
        ...
        <button className="submit-courses-button" type="submit">
          Lisää
        </button>
      </form>
    </div>
